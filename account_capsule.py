...
This is an account of facts, not a trust rating; anyone using it comparatively
must bring their own Sybil resistance. A mesh node's identity is a free,
self-minted keypair (Ed25519 for the ledger, a Nostr key for discovery), so a
node with a poor account can walk away and mint a fresh key with an empty —
...
does NOT bind that identity to a scarce real-world entity. Sybil resistance
(stake, bilateral counterparty attestation, an allow-list, a
proof-of-work/proof-of-personhood layer, …) is entirely on the relying party to
supply. This residual is carried as a first-class field (`sybil_residual`) so
it can never be silently dropped when the account is serialized, forwarded, or
published to Nostr.
"""
...
SYBIL_RESIDUAL_TEXT = (
    "This is an account of facts, not a trust rating; anyone using it "
    "comparatively must bring their own Sybil resistance. A mesh node key is "
    "free to mint, so a node with a poor account can reset to a fresh key with "
    "an empty history at zero cost. The witness makes ONE identity's history "
    "tamper-evident; it does not bind that identity to a scarce real-world "
    "entity. A relying party that needs Sybil resistance must add its own "
    "(stake, counterparty attestation, allow-list, proof-of-personhood) — this "
    "account does not provide it."
)
