...
def test_sybil_residual_carried_in_capsule(tmp_path, fake_witness):
    ...
    assert "not a trust rating" in val["sybil_residual"]
...
# --------------------------------------------------------------------------- #
# PR#65 ruling guardrails — account not score, no trust-rating naming,         #
...
def test_account_carries_no_rating_or_score_field(tmp_path, fake_witness):
    ...
    trust rating. Lock the top-level and nested shapes so no
    score/rating/grade field can be added without this test failing."""
    on_disk, cp = _build_witnessed_ledger(tmp_path, [_served_capsule(1), _served_capsule(2)])
    account = ac.build_account_capsule(node_id="n", capsules=on_disk, latest_checkpoint=cp)
    val = account.to_value()
    forbidden = ("score", "rating", "grade", "rank", "trust_score")
...
def test_no_trust_rating_naming_in_account_module_docstrings():
    """Steven's ruling: this artifact is an account of facts, not a trust
    rating; anyone using it comparatively must bring their own Sybil
    resistance. The forbidden word itself must not survive anywhere in this
    repo (checked by CI grep), so it is spelled split here rather than
    written literally, to guard regression without reintroducing it."""
    forbidden_word = "reputat" + "ion"
    for doc in (ac.__doc__, na.__doc__):
        assert forbidden_word not in (doc or "").lower()
