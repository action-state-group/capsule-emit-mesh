...
| 23 | **SEP-unaware verifier accidentally coupled to `tee_key_custody`** (merge-gate mutant) | A future change makes the pre-existing, SEP-unaware B4 owner-cert verifier (`node_ownership.recheck_ownership_validity`) start consulting the `tee_key_custody` sibling field for its pass/fail verdict — e.g. failing (or upgrading) an otherwise-valid owner cert because the co-signature is tampered. | **CAUGHT** | `test_attack14_sep_unaware_verifier_unaffected_by_tee_key_custody_tamper` | `tee_key_custody` is sealed as a field alongside, never inside, `signed_node_ownership`; `recheck_ownership_validity()`'s signature and body read only the owner-cert bytes. The test reconstructs the SAME `SignedNodeOwnership` from a sealed identity capsule's own bytes, rechecks it, corrupts ONLY `tee_key_custody`'s signature, and rechecks again from the tampered record — asserting the full `OwnershipRecheck` result is byte-for-byte identical. Mutation-tested: wiring a verifier variant that also validates `tee_key_custody`'s signature flips this test to failure (confirmed locally, not shipped). |
