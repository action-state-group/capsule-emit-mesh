...
Bilateral commitment assumes both parties can exchange signatures; on a mesh they often cannot — a
node goes offline, work is batched, an agent hands off asynchronously. The bilateral draft already
specifies the answer, and its discipline is the one this document asks for around anchors: a relay
**MAY** store and forward attestations and issue delivery receipts; it is optional and substitutable,
anyone can run one, they federate, and the role **reads no payloads**; a conformant relay **MUST**
anchor the digests of the receipts it issues to a transparency log it advertises in discoverable
metadata; the log is the relay's choice and the document names none; a relay that will not anchor its
own receipts is non-conformant. A receiving gate **SHOULD** countersign the envelope receipt, so
delivery is a fact both parties assert rather than one the relay asserts alone.
