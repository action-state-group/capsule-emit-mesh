...
- **derivation** — a plain, documented **served/success fold** over the selected
  ...
  not a weighted opinion — it invents no new metric.
...
their own Sybil resistance — identity is free to mint (a mesh node key costs
...
**HELD — no publish to a public relay fired by this repo.** Building and
test-relay-verifying the publish capability is in scope; firing it at a public
relay (relay.damus.io, relay.primal.net, nos.lol, …) is a separate, explicit,
gated act and is **not** done here — that is the open NIP-56 / properties-not-
scores question, revisited after our own test relay / witness host is in place.
...
- `nostr_relay_client.WebsocketRelayClient` (`nostr_relay_client.py`) — a REAL
  ...
  on `127.0.0.1`. `tests/test_nostr_relay_client_and_live_publish.py` covers
  the full round trip plus two live mutants (tampered content → signature
  verify fails; wrong-key signature → relay rejects), and skips if `nak` is
  absent.
- `nostr_live_publish.py` — the **HELD** live-publish path. Same client, but
  `publish_account_capsule_live()` structurally refuses (raises
  `LivePublishHeld`) unless called with BOTH a non-empty relay-URL list AND
  `i_have_stevens_go_ahead_for_a_public_relay=True`. Nothing in this repo ever
  sets that flag together with a real relay list — see
  `tests/test_nostr_relay_client_and_live_publish.py`'s repo-wide guardrail
  test. `describe_live_publish_plan()` builds and signs the exact event a live
  publish would send — kind, tags, content, the node's Nostr pubkey — entirely
  offline, so a human can review it before any gate opens.
...
| statistical fingerprint (step 4, *not* built) | `model_attestation.compute_attestation["x-mesh-poc-v1"].evidence_refs.statistical_fingerprint` | **Typed reference field, present but empty** — exactly Steven's "rides as a typed reference (digest + declared context)" framing. Populating this later upgrades the evidence a record carries without changing the record shape. |
| runtime/binary attestation (**`self_measured`** / **`os_measured`**) | `model_attestation.compute_attestation["x-mesh-poc-v1"].evidence_refs.binary_attestation` | The **runtime/binary attestation rung**: the node hashes the serving binary it actually runs (`std::env::current_exe`) and signs that SHA-256 with the **node key**, shaped like executable code-signing (`{measurement_class, digest, digest_alg, binary_path, binary_size_bytes, signature, signature_algorithm, signer_key_id, measured_at, context}`). **On macOS this upgrades to `os_measured`** (rung 3a): the KERNEL (AMFI) independently reports the running process's code-directory hash (cdhash) via `csops(2) CS_OPS_CDHASH` — a value the process cannot choose — added as `code_directory_hash`/`cdhash_source` and signed JOINTLY with the self-measured digest, so tampering either after signing breaks verification. **HONESTY GRADE — read before trusting it:** `self_measured` proves only that "a process holding the node key reported this hash for the file it believes is its executable"; it does **NOT** prove the binary was un-tampered before it hashed itself. `os_measured` proves an INDEPENDENT (kernel) measurer validated the running bytes' cdhash — but only up to **SIP enabled**, a **non-root** attacker, and the binary's own signing strength: a local `cargo build` carries only an **ad-hoc** signature (`ad_hoc_signature: true`, no Developer-ID/notarization), carried in-band via `ad_hoc_signature`/`signing_authority`/`sip_enabled` and the `context` string — never silently read as `tee_measured` (no hardware register involved). Root+SIP-disabled is the named residual only `tee_measured` closes (`docs/REDTEAM-RUNG3.md`). The label rides IN the signed record either way, so a reader is never told "attested binary" without also being told which grade backs it. When the binary can't be measured the slot is recorded **absent** (`measurement_class: null`) — never fabricated. See `plugins/capsule-producer/src/runtime_attest.rs`. |
| TEE attestation (**`tee_measured`**, rung 3c) | `model_attestation.compute_attestation["x-mesh-poc-v1"].evidence_refs.tee_attestation` | The **`tee_measured` rung**, the strongest measurement grade this codebase names: on an Intel TDX Confidential VM, the TDX module measures MRTD/RTMR[0..3] into hardware registers **before** the guest OS or serving process runs, and produces a hardware-signed TD quote binding a caller-chosen 64-byte REPORTDATA field (here, `SHA-512(domain_tag \|\| capsule_digest \|\| nonce)` — see `tee_attest::tee_binding_report_data`) — shaped `{measurement_class, quote_format, quote, mrtd, rtmr0..rtmr3, report_data, binding_domain_tag, ita_token, measured_at, context}`. **HONESTY GRADE:** unlike `self_measured`, the CPU/TDX module signs the measurement, not the process — a root-compromised host cannot forge it the same way (`docs/REDTEAM-RUNG3.md` attack 16 closes RUNG2 attack 8). It does **NOT** prove Intel's DCAP PKI is uncompromised or that the platform's TCB is revocation-current — verification (`tee_verify::verify_dcap_quote`, cert chain to an OPERATOR-SUPPLIED Intel root, never a hardcoded one; plus an optional `verify_ita_token` Trust Authority path) is purely local/offline and does **not** query Intel's live PCS/ITA collateral, so TCB freshness/revocation is an explicit, named residual (`docs/REDTEAM-RUNG3.md`), not a proof this record makes. **The spec/verifier leg here is hardware-independent** (exercised against synthetic quotes/cert chains in tests); the PRODUCER leg (reading a REAL quote via Linux configfs-TSM on a provisioned TDX node) is HW-gated and, on every host without TDX — including this codebase's Mac development fleet — the slot degrades to **absent** (`measurement_class: null`), never fabricated, honestly capping the ladder at `self_measured`/`os_measured` there. See `plugins/capsule-producer/src/tee_attest.rs` / `tee_verify.rs`. |
